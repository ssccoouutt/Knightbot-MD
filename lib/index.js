...
async function isSudo(userId) {
    try {
        ...
        // Clean the userId - remove device IDs but KEEP @lid for now
        const cleanUserId = userId.split(':')[0];
        
        // Check each sudo entry
        for (const sudoEntry of sudoList) {
            const cleanSudoEntry = sudoEntry.split(':')[0].split('@')[0];
            
            // If userId is LID, compare only the numeric part
            if (userId.includes('@lid')) {
                const lidNumeric = userId.split('@')[0];
                if (lidNumeric === cleanSudoEntry) {
                    console.log('✅ LID numeric match!');
                    return true;
                }
            }
            
            // Normal comparison for phone numbers
            if (cleanUserId === sudoEntry || cleanUserId === cleanSudoEntry + '@s.whatsapp.net') {
                return true;
            }
        }
...
async function addSudo(userJid) {
    try {
        ...
        // Extract JUST the number (remove @lid, @s.whatsapp.net, device IDs)
        const cleanNumber = userJid.split(':')[0].split('@')[0];
        
        if (!data.sudo.includes(cleanNumber)) {
            data.sudo.push(cleanNumber);  // Store only the number!
            saveUserGroupData(data);
        }
        return true;
    } catch (error) {
...
async function removeSudo(userJid) {
    try {
        ...
        const cleanNumber = userJid.split(':')[0].split('@')[0];
        data.sudo = data.sudo.filter(num => num !== cleanNumber);
        saveUserGroupData(data);
        return true;
    } catch (error) {
        return false;
...
// isSudo function can stay the same or be simplified
async function isSudo(userId) {
    try {
        const data = loadUserGroupData();
        const sudoList = data.sudo || [];
        const cleanNumber = userId.split(':')[0].split('@')[0];
