...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    try {
        ...
        // Try normal isSudo check first
        let isSudoUser = await isSudo(senderId);
        
        // If not found and it's a LID, try numeric match
        if (!isSudoUser && senderId.includes('@lid')) {
            console.log('🔄 Trying numeric match for LID...');
            const senderLidNumeric = senderId.split('@')[0].split(':')[0];
            
            for (const sudoEntry of sudoList) {
                const sudoNumber = sudoEntry.split('@')[0];
                if (sudoNumber === senderLidNumeric) {
