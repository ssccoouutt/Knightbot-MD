...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    // Get the REAL JID (with @s.whatsapp.net) instead of LID
    let realJid = senderId;
    
    // If it's a LID and we have group context, try to get the real JID
    if (senderId.includes('@lid') && sock && chatId && chatId.endsWith('@g.us')) {
        try {
            const groupMetadata = await sock.groupMetadata(chatId);
            const participant = groupMetadata.participants.find(p => p.lid === senderId);
            if (participant && participant.id) {
                realJid = participant.id; // This is the real JID with @s.whatsapp.net!
                console.log('✅ Converted LID to real JID:', realJid);
            }
        } catch (e) {
            console.error('❌ Error converting LID to JID:', e);
        }
    }
    ...
    console.log('\n========== OWNER/SUDO CHECK ==========');
    console.log('📱 Original sender:', senderId);
    ...
    const senderIdClean = realJid.split(':')[0].split('@')[0];
    ...
    if (senderId.includes(ownerNumberClean) || realJid.includes(ownerNumberClean)) {
        console.log('✅ Sender ID contains owner number!');
        return true;
    }
    
    // Check sudo status using REAL JID
    console.log('🔍 Checking sudo status...');
    try {
        const sudoList = await getSudoList();
        console.log('📋 Current sudo list:', sudoList);
        
        // Direct check with real JID
        const isSudoUser = sudoList.includes(realJid);
